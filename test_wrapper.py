...
while env.viewer.is_running():
    ...
    if (counter % (sim_frequency / mpc_frequency) == 0 or counter == 0):
        ...
        # time_axis = np.linspace(0, 2 * config.dt, 100)
        # taus = np.zeros((config.n_joints,100))
        # for i in range(100):
        #     taus[:,i] = spline(a, b ,d,time_axis[i])
        # # print(taus)
        # ax.clear()
        # ax.plot(time_axis, taus[0,:], label="tau_sline")
        # ax.plot(config.dt, tau[0], "ro", label="tau")
        # ax.plot(2*config.dt, tau1[0], "ro", label="tau1")
        # ax.plot(0, tau0[0], "ro", label="tau0")
        # ax.set_xlabel("Time (s)")
        # ax.set_ylabel("Value")
        # ax.set_title("Tau Sline and Tau")
        # ax.legend()
        # plt.pause(0.1)
