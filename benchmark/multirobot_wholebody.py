...
for n_robot in robots:
    ...
    def multi_robot_cost(x, u, t, reference):
        return jnp.sum(jnp.array([cost(x[n*i:n*i+n], u[m*i:m+m*i], t, reference[:,reference_size*i:reference_size+reference_size*i]) for i in range(n_robot)]))
    ...
    # noise_dp_ref = jax.random.normal(jax.random.PRNGKey(0), (N + 1, 3 * n_robot)) * 0.001
    ...
    X,U,V = work(reference,parameter,x0_,X0,U0,V0)
    mpx_sum = 0
    for i in range(100):
        start = timer()
        X,U,V = work(reference,parameter,x0_,X0,U0,V0)
        end = timer()
        all_mpx.append(end-start)
        mpx_sum += (end-start)

    print(f"Time for mpx N = {n_robot} is {mpx_sum/100}")
    times_mpx.append(mpx_sum/100)

import matplotlib.pyplot as plt

plt.plot(robots, times_mpx, marker='o')
plt.xlabel('Number of robots in the controller')
plt.ylabel('Average Time (s)')
plt.title('Average Time vs Number of robots')
plt.xscale('log')
plt.yscale('log')
plt.grid(True)
plt.show()

with open('benchmark_multirobot_WB_times.pkl', 'wb') as f:
    pickle.dump({'all_mpx': all_mpx}, f)