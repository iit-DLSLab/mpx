...
#     master_key, _ = jax.random.split(master_key)
#     dp_ref_with_noise = noise_dp_ref
#     noise_x0 = jax.random.normal(master_key,n) * 0.01
#     U0 = jnp.tile(grf_ref, (N, 1))
#     X0 = jnp.tile(x0, (N + 1, 1))
#     V0 = jnp.zeros((N + 1, n ))
#     reference = jnp.tile(jnp.concatenate([p_ref, rpy_ref, noise_dp_ref, omega_ref,grf_ref]), (N + 1, 1))
#     parameter = jnp.tile(jnp.concatenate([jnp.ones(4),p_legs0]),(N+1,1))
#     X,U,V, _,_ = work(reference,parameter,x0+noise_x0,X0,U0,V0)
#     args = {}
#     make_model = True
#     args['N'] = N # Horizon lenght
#     args['dt'] = dt # delta time between the integration node
#     srbd_acados = acd.ocp_formulation(args)
#     srbd_acados_solver = srbd_acados.getOptimalProblem(model_name = "srbd_" + str(N))
#     start = timer()
#     mpx_time = 0
#     acados_time = 0
#     for k in range(N):
#         srbd_acados_solver.set(k, 'p', np.array(jnp.concatenate([parameter[k, :],jnp.array([dt])])))
...
#         stop = timer()
#         mpx_time += stop-start
#         srbd_acados_solver.set(0, 'lbx', np.array(x0+noise_x0))
#         srbd_acados_solver.set(0, 'ubx', np.array(x0+noise_x0))
#         start = timer()
#         srbd_acados_solver.solve()
#         stop = timer()
#         acados_time += stop-start
#     end = timer()
...
for val in Ns:
    ...
    V0 = jnp.zeros((N + 1, n ))
    noise_x0 = jax.random.normal(master_key,n) * 0.01
    reference = jnp.tile(jnp.concatenate([p_ref, rpy_ref, dp_ref, omega_ref,grf_ref]), (N + 1, 1))
    parameter = jnp.tile(jnp.concatenate([jnp.ones(4),p_legs0]),(N+1,1))
    batch_x0 = jnp.tile(x0+noise_x0, (val, 1))
    ...
    X,U,V, _,_ = vmap_work(batch_reference,batch_parameter,batch_x0,batch_X0,batch_U0,batch_V0)
    ...
    args['dt'] = dt # delta time between the integration node
    srbd_acados = acd.ocp_formulation(args)
    srbd_acados_solver = srbd_acados.getOptimalProblem(model_name = "srbd_" + str(N),n_batch=val)
    for i in range(val):
        for k in range(N):
            srbd_acados_solver.ocp_solvers[i].set(k, 'p', np.array(jnp.concatenate([parameter[k, :],jnp.array([dt])])))
            srbd_acados_solver.ocp_solvers[i].cost_set(k,'y_ref',np.array(reference[k, :]))
        srbd_acados_solver.ocp_solvers[i].set(0, 'lbx', np.array(x0+noise_x0))
        srbd_acados_solver.ocp_solvers[i].set(0, 'ubx', np.array(x0+noise_x0))
    start = timer()
    for i in range(10):
        X,U,V, _,_ = vmap_work(batch_reference,batch_parameter,batch_x0,batch_X0,batch_U0,batch_V0)
        X.block_until_ready()
    end = timer()
    print(f"MPX Time for n = {val} is {(end-start)/10}")
    times_mpx.append((end-start)/10)
    start_acados = timer()
    for i in range(10):
        srbd_acados_solver.solve()
    end_acados = timer()
    times_batch_acados.append((end_acados-start_acados)/10)
    print(f"ACADOS Time for n = {val} is {(end_acados-start_acados)/10}")

import matplotlib.pyplot as plt
plt.plot(Ns, times_mpx, marker='o')
plt.plot(Ns, times_batch_acados, marker='o')
