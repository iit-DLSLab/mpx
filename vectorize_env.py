...
p_legs0 = jnp.array([ 0.192, 0.142, 0.024,  -0.195,  0.142,  0.024,-0.195,  0.142,  0.024, -0.195, -0.142, 0.024])
print('leg:\n',p_legs0)
@jax.jit
def reference_generator(t_timer, x,rpy, foot, input, duty_factor, step_freq,step_height,liftoff):
    ...
    def foot_fn(t,carry):
        ...
        foot0 = jnp.array([ 0.192, 0.142, 0.024,  -0.195,  0.142,  0.024,-0.195,  0.142,  0.024, -0.195, -0.142, 0.024])
...
def mpc_step(model, data, X0, U0, V0, t):
    ...
    x0 = jnp.concatenate([mjx_data.qpos[:3],rpy,mjx_data.qvel[:3],mjx_data.qvel[3:6]])
    X_new, U_new, V_new = jax.lax.cond(t % 100 == 0, lambda _: work(reference, parameter, x0, X0, U0, V0), lambda _: (X0, U0, V0), None)
    grf = U_new[0,:3*n_contact]
    tau = J@grf
    data = mjx_data.replace(ctrl=tau[6:])
    return mjx.step(model, data), X_new, U_new, V_new 
def integrate_mpc(model, data, X0, U0, V0):
    ...
    t = jnp.arange(200)
    (data, X0, U0, V0), _ = jax.lax.scan(scan_fn, (data, X0, U0, V0), t, length=200)
